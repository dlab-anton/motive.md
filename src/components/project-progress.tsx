...
export function ProjectProgress({ community, hosted, stale, me, compact = false }: { community: ParticipationPublicProjection | null; hosted: HostedCirclePublicResults | null; stale: boolean; me: ParticipationMeResponse | null; compact?: boolean }) {
  ...
  return <section className={`project-progress research-overview${compact ? ' research-overview-compact' : ''}`} aria-labelledby="project-progress-title">
    ...
    {compact && activeIntents.length ? <div className="research-current-questions">
      ...
      {activeIntents.slice(0, 2).map(intent => <a key={intent.claimId} href={`${projectLink(slug)}&tab=updates#active-research`}><span>{researchQuestionExcerpt(intent.proposal)}</span><span className="research-question-author">{intent.agentName}{intent.contributorDisplayName ? ` · ${intent.contributorDisplayName}` : ''}<ArrowUpRight aria-hidden="true" /></span></a>)}
      {activeIntents.length > 2 ? <a className="inline-link" href={`${projectLink(slug)}&tab=updates#active-research`}>See more shared questions <ArrowUpRight aria-hidden="true" /></a> : null}
    </div> : null}
    {!compact ? activeIntents.length ? <div className="active-research-intents">
      ...
      {activeIntents.length > 3 ? <a className="inline-link" href={`${projectLink(slug)}&tab=updates`}>See other active questions <ArrowUpRight /></a> : null}
    ...
    {!compact ? <><a className="inline-link research-journal-link" href={`${projectLink(slug)}&tab=updates`}>Follow the questions and discoveries <ArrowUpRight /></a>
