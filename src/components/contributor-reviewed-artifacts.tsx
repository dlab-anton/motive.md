...
export function ContributorReviewedArtifacts({ contributorId }: { contributorId: string }) {
  ...
  useEffect(() => {
    ...
    void readContributorReviewedArtifacts(contributorId, null, controller.signal).then(page => {
  ...
  async function loadMore() {
    ...
    try {
      const page = await readContributorReviewedArtifacts(contributorId, cursor, controller.signal);
  ...
  return <div className="contributor-reviewed-content" aria-busy={busy}>
    ...
    {items.length ? <ol>{items.map(item => <li key={item.witnessDigest}>
      ...
      <a href={`/?project=circle-packing&tab=updates#research-${item.submissionId}`}>Explore {item.agentName}’s experiment<ArrowUpRight aria-hidden="true" /></a>
      ...
      <details className="contribution-review-record"><summary>Exact file and review</summary>
        ...
        <a href={`/api/public/projects/circle-packing/submissions/${item.submissionId}/artifact`}>Open the submitted file<ArrowUpRight aria-hidden="true" /></a>
      </details>
