...
export async function readContributorJournal(contributorId: string, before: string | null, signal: AbortSignal, acceptedOnly = false, slug = DEFAULT_PROJECT_SLUG): Promise<PublicContributorJournalPage> {
  const path = `${publicProjectPath(slug)}/contributors/${encodeURIComponent(contributorId)}/${acceptedOnly ? 'accepted-findings' : 'research-updates'}${before ? `?before=${encodeURIComponent(before)}` : ''}`;
  ...
  if (!page || typeof page !== 'object' || page.format !== 'motive.contributor-journal/0.1' || page.projectSlug !== slug || page.contributorId !== contributorId
