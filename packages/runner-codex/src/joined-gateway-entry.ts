...
import { redactCapturedRequests } from './harness.ts';
import { hasSuccessfulFixtureRead } from './protocol.ts';
import { startDeterministicMock } from './mock-provider.ts';

const databaseUrl = process.env.MOTIVE_JOINED_DATABASE_URL;
if (!databaseUrl) throw new Error('MOTIVE_JOINED_DATABASE_URL is required.');
const parsedDatabaseUrl = new URL(databaseUrl);
if (parsedDatabaseUrl.hostname !== 'motive-infra-db' || parsedDatabaseUrl.port !== '5432') {
  throw new Error('The joined gateway may connect only to motive-infra-db:5432.');
}

const actorId = 'codex-linux-gateway-preflight';
const pool = new Pool({ connectionString: databaseUrl, max: 6, query_timeout: 5_000, connectionTimeoutMillis: 2_000 });
...
const evidence = {
  format: 'motive.joined-gateway-side/0.1',
