...
const toolImage = 'motive-evaluator-tools:namespace-lock-review-r2';
const guestImage = 'motive-evaluator-systemd-qemu:namespace-lock-review-r2';
...
const name = `motive-evaluator-namespace-${randomBytes(6).toString('hex')}`;
const outputPrefix = 'fixtures/compatibility/evidence/evaluator-namespace-qemu';
...
const sourceFiles = [
  ...
  'packages/evaluator-lean/native/run-namespace-qemu-review.ts',
...
const toolDigests = await checked(['run', '--rm', '--network', 'none', '--read-only', '--cap-drop', 'ALL',
  ...
  '/opt/evaluator/bin/comparator', '/opt/evaluator/bin/landrun', '/opt/evaluator/bin/lean4export',
...
const resultLines = [...cleanLog.matchAll(/^MOTIVE_EVALUATOR_GUEST_RESULT=(\{[^\n]+\})$/gm)];
...
const cases = [...cleanLog.matchAll(/^case=([a-z-]+) exit=(\d+) log_sha256=([a-f0-9]{64})$/gm)]
  .map(match => ({ id: match[1], exitCode: Number(match[2]), logDigest: `sha256:${match[3]}` }));
const expectedCases = ['valid-proof', 'wrong-target-statement', 'incomplete-proof', 'unapproved-custom-axiom',
  'transitive-incomplete-dependency', 'forged-acceptance-output'];
if (cases.length !== expectedCases.length || !expectedCases.every(id =>
  cases.filter(item => item.id === id && item.exitCode === (id === 'valid-proof' ? 0 : 1)).length === 1)) {
  diagnostics.push('Expected exact valid and negative fixture exits were not observed.');
}
const probes = {
...
if (guestResult?.status !== 'BEHAVIORAL_CASES_PASSED' || guestResult?.cases !== 8
...
const evidence = { format: 'motive.evaluator-namespace-qemu-review/0.1', capturedAt: new Date().toISOString(),
  status: diagnostics.length === 0 && removed ? 'PASSED_LOCAL_NAMESPACE_FIXTURES' : 'FAILED',
  ...
  logDigest: sha256(log), logBytes: Buffer.byteLength(log), guestResult, cases, probes, diagnostics,
...
console.log(JSON.stringify({ status: evidence.status, cases, probes, diagnostics, outputPrefix }, null, 2));
